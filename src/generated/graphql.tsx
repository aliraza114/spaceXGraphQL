...
export type LaunchesQuery = (
  ...
  & { launches?: Maybe<Array<Maybe<(
    { __typename?: 'Launch' }
    & Pick<Launch, 'flight_number' | 'launch_year' | 'mission_name' | 'launch_success'>
  )>>> }
);

export type FlightInfoQueryVariables = Exact<{
  id?: Maybe<Scalars['String']>;
}>;


export type FlightInfoQuery = (
  { __typename?: 'Query' }
  & { launch?: Maybe<(
    { __typename?: 'Launch' }
...
export const FlightInfoDocument = Apollo.gql`
    query flightInfo($id: String) {
  launch(id: $id) {
    flight_number
    mission_name
    launch_year
...
/**
 * __useFlightInfoQuery__
 *
 * To run a query within a React component, call `useFlightInfoQuery` and pass it any options that fit your needs.
 * When your component renders, `useFlightInfoQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useFlightInfoQuery({
 *   variables: {
 *      id: // value for 'id'
 *   },
 * });
 */
export function useFlightInfoQuery(baseOptions?: Apollo.QueryHookOptions<FlightInfoQuery, FlightInfoQueryVariables>) {
        return Apollo.useQuery<FlightInfoQuery, FlightInfoQueryVariables>(FlightInfoDocument, baseOptions);
      }
export function useFlightInfoLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<FlightInfoQuery, FlightInfoQueryVariables>) {
          return Apollo.useLazyQuery<FlightInfoQuery, FlightInfoQueryVariables>(FlightInfoDocument, baseOptions);
        }
export type FlightInfoQueryHookResult = ReturnType<typeof useFlightInfoQuery>;
export type FlightInfoLazyQueryHookResult = ReturnType<typeof useFlightInfoLazyQuery>;
export type FlightInfoQueryResult = Apollo.QueryResult<FlightInfoQuery, FlightInfoQueryVariables>;